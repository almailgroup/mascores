...
export function SeasonMenu({ seasons, value, onChange, footer, onHero = false }: {
  ...
  return <div ref={root} className="relative z-[60] shrink-0">
    ...
    {/* Opens from the button's own edge and is width-capped, so it can never run off a phone screen. */}
    {open && <div role="listbox" className="absolute start-0 z-[70] mt-2 max-h-72 w-44 max-w-[70vw] overflow-y-auto rounded-xl border border-border bg-card p-1 text-foreground shadow-2xl">
