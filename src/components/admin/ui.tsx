...
export function Modal({ open, onClose, title, children, wide }: { open: boolean; onClose: () => void; title: string; children: ReactNode; wide?: boolean }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto overscroll-contain bg-black/60 p-2 backdrop-blur-sm sm:p-4" onClick={onClose}>
      <div className={`my-3 w-full min-w-0 ${wide ? "max-w-4xl" : "max-w-lg"} rounded-3xl border border-border bg-card p-4 shadow-2xl sm:my-8 sm:p-6`} onClick={(e) => e.stopPropagation()}>
