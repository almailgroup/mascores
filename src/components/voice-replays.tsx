...
export function VoiceReplays({ hostId, matchId, mine = false, limit = 12 }: { hostId?: string; matchId?: string; mine?: boolean; limit?: number }) {
  const tx = useTx();
  const { user } = useAuth();
  ...
  const replays = useQuery({
    queryKey: ["voice-replays", owner ?? null, matchId ?? null, limit],
    enabled: !mine || !!user,
    queryFn: async () => {
      let query = supabase
        .from("voice_recordings")
        .select("id, host_id, title, cover_url, audio_url, duration_seconds, created_at, match_id")
