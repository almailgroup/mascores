...
function MatchPage() {
  ...
  return (
    <AppShell>
      ...
      {tab === "lineups" && <div className="grid gap-4 md:grid-cols-2">{[match.home, match.away].map((team) => <div key={team?.id ?? "tbd"} className="rounded-2xl border border-border bg-card p-4"><h3 className="mb-3 font-bold">{team?.name ?? "TBD"}</h3>{lineups.data?.filter((item) => item.team_id === team?.id).map((lu) => <Link key={lu.id} to="/players/$id" params={{ id: lu.player_id }} className="flex items-center gap-3 border-t border-border py-2 first:border-0"><div className="h-9 w-9 overflow-hidden rounded-full bg-muted">{lu.player?.photo_url && <img src={lu.player.photo_url} alt="" className="h-full w-full object-cover" />}</div><span className="w-6 text-xs text-muted-foreground">{lu.shirt_number ?? lu.player?.shirt_number}</span><span className="font-semibold">{lu.player?.name}</span><span className="ml-auto text-xs text-muted-foreground">{lu.is_starting ? lu.position_code ?? "XI" : "Bench"}</span></Link>)}{!lineups.data?.some((item) => item.team_id === team?.id) && <p className="text-sm text-muted-foreground">No lineup posted.</p>}</div>)}</div>}
